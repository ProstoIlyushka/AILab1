...
void Game::updateScales() {
    auto calculateScaleAndOffset = [this](sf::Texture& texture, float& scale, sf::Vector2f& offset, bool useAuto) {
        if (useAuto) {
            // Автоматический расчёт (как было)
        ...
        // else: оставляем scale и offset как есть (ручные значения)
        };

    // Для каждого типа объекта решаем, использовать авто или ручной режим
    calculateScaleAndOffset(playerTexture, playerScale, playerOffset, true);  // игрок авто
    calculateScaleAndOffset(boxTexture, boxScale, boxOffset, true);          // ящик авто
    calculateScaleAndOffset(wallTexture, wallScale, wallOffset, false);      // стена - ручной
    calculateScaleAndOffset(targetTexture, targetScale, targetOffset, false); // цель - ручной
    calculateScaleAndOffset(floorTexture, floorScale, floorOffset, true);     // пол авто
...
void Game::loadLevelFromData(const LevelData& data) {
    ...
    // Выводим карту в консоль для наглядности
    ...
    while (window.pollEvent(event)) {
        if (event.type == sf::Event::Closed) {
            ...
            // Генерация карты
                ...
                Test tester(*this);
                tester.runAllTests();      // все тесты
                // tester.runUnsolvableTest();  // тест на нерешаемую задачу
            }
            // Статистика
